...
- **Revisión manual asistida de los 384 servicios**: aliste.info lleva años sin actualizarse (ningún servicio tiene `sitio_web` para poder comprobarlo automáticamente), así que la única forma fiable de saber qué negocios siguen existiendo es revisarlos uno a uno. Cada fila de `/admin/servicios` tiene un botón "Ver en Google Maps" (busca por nombre + pueblo, `Servicio::enlace_maps`) y un botón "Marcar como revisado" que guarda `revisado_en`; el filtro "Ocultar ya revisados" permite ir avanzando por la lista en varias sesiones sin repetir trabajo.
...
El enlace "Administración" (visible para administrador y redactor, `@can('redactar-noticias')`) solo aparece en el navbar del **backoffice** (`livewire/layout/navigation.blade.php`, compartido por `layouts/app.blade.php` y `layouts/admin.blade.php`) — **no** en el navbar del sitio público (`layouts/public.blade.php`). En móvil se muestra directamente en la barra superior (junto al botón de menú hamburguesa), no dentro del menú desplegable.
...
- **Cada fallo de envío de una notificación push** (suscripción caducada, error del servicio push, etc.) se registra como `error` con origen `notificaciones:push`, enganchado al evento `NotificationChannels\WebPush\Events\NotificationFailed` en `AppServiceProvider::boot()`. Es necesario porque el paquete de WebPush **no lanza excepción** cuando un envío falla — sin este listener esos fallos eran invisibles, ni en este log ni en ningún otro sitio. El propio listener va envuelto en un `try/catch` (igual que el manejador global de `bootstrap/app.php`): si registrar el fallo también fallase, cae al log de ficheros en vez de romper el flujo que disparó la notificación (p. ej. un registro de usuario).
- **`logs.mensaje` es `TEXT`, no `VARCHAR`** — un `VARCHAR(255)` (y hasta un `VARCHAR(1000)`) se queda corto para mensajes largos: `Str::limit($texto, 1000)` trunca el contenido a 1000 caracteres pero **añade además el sufijo "..."**, superando cualquier límite fijo por poco. Un motivo de fallo de push largo llegó a romper en cascada el registro de un usuario en producción por esto exactamente (`SQLSTATE[22001]: Data too long for column 'mensaje'`, lanzado dentro del propio listener de logging, no capturado en ese momento).
...
- `NuevoUsuarioRegistrado`: aviso automático a todos los administradores con suscripción push cada vez que se registra un usuario nuevo (tanto por email/contraseña como por Google), con el texto "Nombre de Pueblo se ha registrado" (o solo "Nombre se ha registrado" si no eligió pueblo). Enganchado al evento nativo de Laravel `Illuminate\Auth\Events\Registered` vía `App\Listeners\NotificarAdminNuevoRegistro` (registrado en `AppServiceProvider::boot()`), no a cada formulario por separado. En producción se observó que Livewire puede disparar este evento dos veces dentro de la misma petición HTTP (causa no determinada); el listener es idempotente vía `users.notificacion_registro_enviada_en` (un `UPDATE ... WHERE NULL` atómico) para que, ocurra lo que ocurra, cada usuario solo genere un aviso.
...
- **`escapeshellarg()`/`exec()` pueden estar deshabilitadas en producción (Plesk).** El comando `backup:base-datos` fallaba en PROD con "Call to undefined function escapeshellarg()" porque el hosting deshabilita esa función por seguridad vía `disable_functions` en el `php.ini`. Se corrigió pasando el comando a `Process::run()` como **array** (`['mysqldump', '--single-transaction', ...]`) en vez de como string armado a mano — Symfony Process escapa los argumentos internamente sin necesitar `escapeshellarg()` ni invocar un shell — y comprimiendo la salida con `gzopen()`/`gzwrite()`/`gzclose()` (zlib) en vez de canalizarla a un `gzip` externo por un pipe de shell.
- **Deduplicación de noticias por similitud de título, no solo por URL.** `ScrapearNoticiasCommand` ya evitaba reimportar la misma URL exacta, pero eso no bastaba para el mismo suceso contado con otras palabras (p. ej. por una fuente distinta en el futuro). Antes de importar, compara el título normalizado (`Str::ascii()->lower()->squish()`) contra los títulos de las noticias publicadas en los últimos 3 días con `similar_text()`; si la similitud es ≥85% se descarta como probable duplicado. Se investigaron dos fuentes adicionales (Zamora News y NoticiasZamora.es) pero ninguna resultó viable: la etiqueta "comarca-de-aliste" de Zamora News apenas tiene contenido propio y mezcla noticias genéricas de toda la provincia, y NoticiasZamora.es bloquea las peticiones automatizadas (403 vía CloudFront).
- **`Laravel\Socialite\Two\InvalidStateException` en el login con Google es un caso esperable, no un bug a eliminar.** Salta cuando el `state` de la sesión no coincide con el de la URL de callback (sesión caducada, doble clic, o alguien visitando `/auth/google/callback` directamente sin pasar antes por `/auth/google/redirigir`). `GoogleAuthController::callback()` la captura específicamente y redirige a `/iniciar-sesion` con un aviso (reutilizando `<x-auth-session-status>`) en vez de dejarla reventar como error no controlado.
