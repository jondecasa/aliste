...
class ServicioSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($pagina = 1; $pagina <= $totalPaginas; $pagina++) {
            $this->command?->info("Descargando página {$pagina} de {$totalPaginas}...");
        ...
        $this->command?->info("Servicios creados: {$creados}");
        $this->command?->info("Servicios actualizados: {$actualizados}");

        if ($pueblosSinMatch !== []) {
            $this->command?->warn('Pueblos sin correspondencia: '.implode(', ', array_keys($pueblosSinMatch)));
        }

        if ($categoriasSinMatch !== []) {
            $this->command?->warn('Categorías sin correspondencia: '.implode(', ', array_keys($categoriasSinMatch)));
        }
    }
